...
(defun start-systray (&optional (host "") (dpy nil))
  (ensure-config-file-exists)
  (let* ((display (open-display host :display dpy))
	 (screen (display-default-screen display))
	 (root-window (screen-root screen))
	 (depth (drawable-depth root-window))
	 (tray-height 30)
         (other-win (xlib:create-window :parent root-window
					:x 0 :y 0
					:depth depth
					:width 22
					:height tray-height
					:background (alloc-color (window-colormap root-window) (xlib:make-color))
	...
	(do () (nil)
	  (display-finish-output display)
	  ;; (process-event display :discard-p t
	  ;; 		 :handler (lambda (&rest args &key &allow-other-keys)
	  ;; 			    (format t "--> ~S~%" args)))
