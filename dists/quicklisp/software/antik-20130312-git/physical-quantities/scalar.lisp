...
;; Time-stamp: <2011-10-08 18:00:07EDT scalar.lisp>
...
(defun make-pq
    (magnitude unit-expression
     &optional (sysunits *system-of-units*) (scalar-dimension t))
  ...
  (if (and (physical-quantity-p magnitude)
      ...
      (let ((mag
		 ...
		 (funcall (grid:elementwise (alexandria:rcurry #'pqval sysunits))
			  magnitude)
		 magnitude)))
	(if (and (grid:gridp mag) (not scalar-dimension))
	    (make-pq-grid
	     ...
	     sysunits)
	    (make-pq-ue mag unit-expression sysunits)))))
...
(defun make-pq-ue (magnitude unit-expression sysunits)
