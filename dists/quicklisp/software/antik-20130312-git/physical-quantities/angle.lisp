...
;; Time-stamp: <2011-08-03 11:01:07AKDT angle.lisp>
...
(export '(norm-denorm-angle angle-as-time dms-angle time-as-angle write-dms
	  angle-degrees))

;;; Use degrees in the current system of units.
(setsys angle-degrees nil degree)
...
;;; (angle-as-time *)
;;; PT21h48m0.000s
(defun dms-angle (degrees-or-hours minutes seconds &optional hours)
  ...
  (let ((decimal
    ...
    (if hours
	(antik:* (make-pq decimal 'hours) #_1_revolution/day)
...
(defun write-dms
  ...
  (let ((*print-circle* nil))
    (multiple-value-bind (hours hfrac)
	(cl:floor (pqval (antik:abs angle) 'degree *siu*))
      (multiple-value-bind (mins mfrac)
	  (cl:floor (cl:* 60 hfrac))
	(format stream "~2,'0d~a~2,'0d~a~6,3,,,'0f~a"
		(cl:* (cl:round (cl:signum (pqval angle))) hours) first-sep
		mins second-sep
		(cl:* 60 mfrac) final)))))
