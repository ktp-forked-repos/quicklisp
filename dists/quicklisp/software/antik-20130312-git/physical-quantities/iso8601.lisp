...
;; Time-stamp: <2011-05-23 22:28:35EDT iso8601.lisp>
...
(defun iso8601-string
  ...
  (flet ((t-for-zero (integer indicator value)
	   "If indicator and value are eq, and integer is 0,
   return t, otherwise return integer."
	   (or (and (zerop integer) (eq indicator value)) integer)))
    (let ((year (t-for-zero (timeparse-year timeparse) required-component :year))
	  (month (t-for-zero (timeparse-month timeparse) required-component :month))
	  (day (t-for-zero (timeparse-day timeparse) required-component :day))
	  (hour (t-for-zero (timeparse-hour timeparse) required-component :hour))
	  (minute (t-for-zero (timeparse-minute timeparse) required-component :minute))
