...
;; Time-stamp: <2011-10-10 10:41:27EDT linear-algebra.lisp>
...
(defmethod *i ((a grid:foreign-array) (b number))
  (gsl:elt* (grid:copy-to a 'grid:foreign-array) (coerce b (grid:element-type a))))

(defmethod *i ((a number) (b grid:foreign-array))
  (gsl:elt* (grid:copy-to b 'grid:foreign-array) (coerce a (grid:element-type b))))

(defmethod /i ((a grid:foreign-array) (b number))
  (gsl:elt* (grid:copy-to a 'grid:foreign-array)
	    (cl:/ (coerce b (grid:element-type a)))))

(defmethod +i ((a grid:foreign-array) (b grid:foreign-array))
  (gsl:elt+ (grid:copy-to a 'grid:foreign-array) b))
...
(defmethod +i ((a number) (b grid:foreign-array))
  (gsl:elt+ (grid:copy-to b 'grid:foreign-array)
...
(defmethod -i ((a grid:foreign-array) (b number))
  (gsl:elt+ (grid:copy-to a 'grid:foreign-array) (cl:- b)))
...
(defun invert-matrix (matrix)
  ...
  (multiple-value-bind (LU permutation signum)
      (gsl:LU-decomposition (grid:copy-to matrix 'grid:foreign-array))
    (values
...
(defun solve-linear (matrix vector)
  ...
  (multiple-value-bind (upper permutation signum)
      (gsl:LU-decomposition (grid:copy-to matrix))
